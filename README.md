...
Programmer Chat Programming Robot(PCPR in short), powered by AI, detects logical errors in your code and provide useful advice.  
...
## Usage
- Open a workspace folder(mandatory).
- Ensure that you have the baseURL, API key, and name of the AI model provided by the AI provider.
- Run **PCPR Check Code** on the command panel(`F1` or `Ctrl/CMD+Shift+P`), and modify the configuration if it is not configured.
- If you want to modify the configuration, please run **PCPR Modify Config** and fill in the baseURL, API key, and AI model.
...
## Attention
All inputs that escaped by `Esc` or are truly empty string inputs are considered as empty string inputs, and the corresponding configuration will be updated to empty.

---

## Behavior
After running **PCPR Check Code**, a prompt *PCPR: Analyzing...* will appear in the bottom left corner, indicating that the AI is analyzing the code. After the analysis is completed, the AI's response will be integrated into a file named **pcpr_desult + current ISO format time.md** in the workspace you are currently opening, and a new markdown preview window will be opened to display the response.

---

## Features
- Focus on analyzing logical errors in your code and providing solutions.
- Integrating the capabilities of external AI into the VSC development process.
- Expanded the available AI models in VSC development, and almost all models that provide API keys can be integrated into the development process for use.
...
- 打开一个工作区文件夹（必须）。
- 确保您拥有AI提供商提供的baseURL, API key和AI模型的名称。
- 在命令面板（`F1`或`Ctrl/Cmd+Shift+P`）运行**PCPR Check Code**，如果没有配置，则修改配置。
- 如果想修改配置，请运行**PCPR Modify Config**并填写baseURL, API key和 AI model。
- 如果无需修改baseURL和API key，仅需修改使用的AI model，请运行**PCPR Change Model**并填写想要使用的模型。

## 注意
所有`Esc`跳出的输入或真正的空字符串输入都被认为是空字符串输入，将会把对应配置更新为空。

---

## 行为

运行**PCPR Check Code**后，左下角会出向*PCPR: Analying....*的提示，表示AI正在分析代码，分析完成后，AI的回复将会被整合到您当前打开的工作区下名为**pcpr_result + 当前的ISO格式时间.md**的文件里，并打开一个新的markdown预览窗口来显示回答。
